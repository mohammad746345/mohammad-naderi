    ...
    useEffect(() => {
        axios({
            ...
            url: `${API_Course_List_all}?status=open`,
                ...
                activeCourses && activeCourses.map((item) => setTimer(item['id'], item['secend_date']['end_date']))
                            ...
                            <Card style={{height: "366px", width: "448px", borderRadius: "10px"}}>
                                <Card.Body className="h-100 p-0">
                                    ...
                                    <div className="col-12 d-flex flex-row bg-light" style={{height:"76px"}}>
                                        <Button className="cardButtonHomeCourses col-5" onClick={() => history.push({pathname:`/course/${item['id']}/`, state:{id:`${item['id']}`}})}>مشاهده دوره</Button>
    ...
    <div>
        <Card className="col-12 h-100 bg-transparent border-0">
            <Card.Header className="col-12 d-flex bg-transparent border-0"
                         style={{fontSize: "23px", fontWeight: "bold", marginTop: "127px", paddingRight: "9%"}}>دوره
